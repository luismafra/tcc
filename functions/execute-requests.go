...
func startCRIUServer(cfg *ExecutionConfig, appDir string, serverLogFile string) (*exec.Cmd, io.ReadCloser, error) {
	...
	upServerCmd.Env = append(upServerCmd.Env, cfg.EnvVars...)
...
func startDefaultServer(cfg *ExecutionConfig, runtime string, appDir string, args string) (*exec.Cmd, io.ReadCloser, io.ReadCloser, error) {
	fmt.Fprintln(os.Stderr, "Default Handler Type")
	var upServerCmd *exec.Cmd
	if runtime == "java" {
		if args != "" {
			upServerCmd = exec.Command("java", "-jar", args, args)
		} else {
			upServerCmd = exec.Command("java", "-jar", fmt.Sprintf("%s/target/app-0.0.1-SNAPSHOT-jar-with-dependencies.jar", appDir), args)
		}
...
func main() {
	...
	if handlerType == "criu" {
		upServerCmd, serverStdout, err = startCRIUServer(cfg, appDir, optPath)
	} else {
		upServerCmd, serverStdout, serverStderr, err = startDefaultServer(cfg, runtime, appDir, optPath)
	}
	...
	fmt.Fscanf(serverStdout, "T4: %d", &mockVar)
	fmt.Fscanf(serverStdout, "T6: %d", &mockVar)
	if classLoadStats {
		fmt.Fscanf(serverStdout, "LCTime: %d", &mockVar)
		fmt.Fscanf(serverStdout, "ICTime: %d", &mockVar)
		fmt.Fscanf(serverStdout, "LC: %d", &mockVar)
		fmt.Fscanf(serverStdout, "TLTime: %d", &mockVar)
	}
	...
	roundTripTime, serviceTime := getRoundTripAndServiceTime(cli, cfg.Requests, req, serverStdout)
...
func getRoundTripAndServiceTime(cli *http.Client, reqs int, req *http.Request, serverStdout io.ReadCloser) ([]int64, []int64) {
