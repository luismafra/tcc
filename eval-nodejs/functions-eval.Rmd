...
```{r pressure, echo=FALSE}
data <- read_startup("function_startup.csv")

ggplot(data, aes(x = app, y = value, color = technique)) +
  geom_boxplot() +
  labs(color = "Técnica", y = "Tempo de inicializar (ms)", x = "") +
  scale_x_discrete(limits = c("NOOP", "Markdown", "Image Resizer")) +
  theme_bw() +
  ggtitle("") +
...
```{r pressure, echo=FALSE}
data_service <- read_service_time("function_startup.csv")
...
```{r pressure, echo=FALSE}
data_request <- get_first_request_time_no_mean(read.csv("function_startup.csv"))
...
ggplot(cold_request_time %>% filter(app == "NOOP"), aes(x = app, y = total, color = technique)) +
  geom_boxplot() +
  labs(color = "Técnica", y = "Cold Start (ms)", x = "") +
  theme_bw() +
  scale_x_discrete(limits = c("NOOP")) +
  ggtitle("") +
  theme(legend.position = "top")
```

## First Request AND Startup-time
